...
import pandas as pd
import numpy as np
from .dbmodules import dbModules as dbm
...
def init_callbacks(dash_app):
    @dash_app.callback(Output("loading-output-2", "children"), [Input("t_status", "value")])
    def input_triggers_nested(value):
        print(value)
        # time.sleep(1)
        return value

    @dash_app.callback([Output("load-val", "style"),
    ...
    @dash_app.callback([Output('data-slider', 'min'),
    ...
    @dash_app.callback([Output('dropdown_table_rangeColumn', 'options'),
    ...
    @dash_app.callback(Output('dropdown_table_filterColumn', 'options'),
                  [Input('upload-data', 'contents'),
                   Input('upload-data', 'filename')])
    ...
    @dash_app.callback(Output('t_status', 'children'),
                  [Input('upload-data', 'contents')])
    ...
    @dash_app.callback([Output('tabs-example-content', 'figure'),
    ...
    @dash_app.callback([Output('dropdown_table_filterColumn', 'value'),
    ...
    def update_output(n_clicks, ses_code, contents, filename):
        ...
        if n_clicks > 0:
            ...
            filter_column, range_column, _range, _lower, _upper, plot_column, ses_code = [settings[x] for x in
                                                                                          params_list]
